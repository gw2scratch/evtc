...
namespace GW2Scratch.ArcdpsLogManager.Logs.Naming
{
	public class TranslatedLogNameProvider : ILogNameProvider
	{
		...
		private async Task GetMapNames()
		{
			...
			// This class has no UI dependency; the event below is raised on a background thread
			// (we got here via Task.Run). UI-layer subscribers are responsible for marshalling
			// onto their UI thread (e.g. via Dispatcher.UIThread in the Avalonia app).
